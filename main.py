...
# --- 1. WEB SERVER ĐỂ GIỮ UPTIME TRÊN RENDER ---
app = Flask('')

@app.route('/')
def home():
    return "Hệ thống đang oanh tạc! Bot is Online."

def run():
    # Render yêu cầu chạy trên port 8080 hoặc port được cấp
...
# --- 2. DANH SÁCH 70+ CÂU CHAT WAR GẮT ---
MESSAGES = [
    ...
    "# Loại_mày_vĩnh_viễn_chỉ_là_kẻ_bám_đuôi! =)))", "# Đừng_tỏ_ra_nguy_hiểm_khi_mày_rỗng_tuếch!",
    "# Bố_mày_khóa_mồm_mày_bằng_tốc_độ_ánh_sáng!", "# Mày_cay_không_con_trai? Cay_cũng_không_làm_gì_được!",
    ...
    "# Đừng_để_bố_phải_dùng_đến_hàng_thật_nhé! =)))", "# Trình_đao_mày_chỉ_đủ_để_cắt_đậu_phụ!",
        ...
        if channel:
            ...
            while True:
                try:
                    ...
                    # Tạo mã ID ngẫu nhiên dài hơn để tránh trùng lặp tin nhắn
                    rand_str = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
                    
                    # Gửi tin nhắn và tag mục tiêu
                    await channel.send(f"<@{target_id}> {msg} | `[REF-{rand_str}]`")
                    
                    # Tốc độ ngẫu nhiên từ 0.5 đến 0.8 để tránh bị Discord Ban nhanh
                    ...
                    if e.status == 429: # Bị Discord chặn tạm thời
                        ...
                        print(f"Bị chặn! Nghỉ {wait_time}s...")
                        await asyncio.sleep(wait_time)
                    else:
                        print(f"Lỗi gửi tin: {e}")
                        await asyncio.sleep(5)
        else:
            print("LỖI: Không tìm thấy Channel ID. Kiểm tra lại quyền của Bot!")
    else:
        print("LỖI: Thiếu cấu hình Environment (CHANNEL_ID hoặc TARGET_USER_ID)!")

# --- 4. KHỞI CHẠY ---
if __name__ == "__main__":
    keep_alive() # Chạy web server để Render không tắt bot
